...
### Three vessel codes, and which one answers which question

Master Vessel now carries a code per system, because one column could not honestly serve all three
uses. Asked by Ryan on 2026-09-23, ahead of the DHM integration and a later jetty planning system.

| Column | Issued by | Cardinality | Answers |
| --- | --- | --- | --- |
| `master_vessels.vessel_code_klip` | KLIP (`KVSL-0001`) | exactly one, immutable | "which vessel is this" |
| `master_vessel_code_aliases` where `namespace='SAP'` | SAP | **zero or more** | "which vessel does this PO mean" |
| `master_vessels.dhm_code` | DHM | one | the key shared with DHM, and later jetty planning |

**Why KLIP needed its own.** `master_vessels.vessel_code` was never a KLIP code - it held whichever
code arrived first (Jovin, then the KLIP sheet, then SAP, then a `TMP-<name>` slug invented when
nothing was known). 295 of 474 vessels held a `TMP-` code, and `displayVesselCode()` renders those
as blank, so 62% of the master showed no Vessel Code at all. `vessel_code_klip` is generated by a
column DEFAULT over a sequence, not in TypeScript, because `master_vessels` is written from four
directions - SAP import, Jovin import, the DHM webhook and the edit modal - and one of them would
eventually skip a helper. It encodes nothing: vessel names will start syncing from DHM, and a code
derived from a name has to change when the name does, which is what a system code exists to
prevent. `TMP-` proved the point - the name was in the code, so the code was treated as temporary.

**Why SAP is a set, not a column.** SAP issues a vessel code per tug/barge combination, and for 22
of the 40 compound names in production the code is taken from the **tug**, not from the barge
carrying the cargo: `MOPTI777` is `TB. OPTIMUS 777 / BG. LUMINOR 6`. So one barge accumulates a
code per tug that ever towed it - `BG. AS MARINA 12` holds three - and a code cannot be parsed to
find its vessel. The mapping table is the authority; the code string is not evidence.

`source` on that table cannot stand in for `namespace`. It records where KLIP first saw a code, not
which system owns it: `source='db_existing'` held 84 genuine SAP codes alongside 382 that were not.

**What the cleansing corrected** (migrations 181-183, from `Vessel Cleanup (Jovin, Klip, SAP).xlsx`,
which Ryan named the source of truth). The workbook treats tug and barge as separate vessels, each
with its own codes, and that model is now KLIP's. Nine codes sat on the wrong vessel; for three of
them SAP's own vessel name agreed with the workbook rather than with KLIP - `MBERLIAN` (49 SAP rows
named BERLIAN PACIFIC III) was filed under `MT. GIAT ARMADA 02`. Where the workbook listed one code
against two vessels, the Jovin-sourced row wins; that settled six of eight, and the last two on
attribute completeness. Net effect: 7 vessels created, 5 duplicate pairs merged, 94 shipments
re-attributed, and every code in `sap_processed_data` now resolves to a vessel - three did not.

Migration 182 widened `normalize_vessel_name()` to strip `SPOB.` and `TKG.`, and to count a
tongkang segment as cargo. Two faults fell out of that: `TK` was tried before `TKG`, so `TKG. PON 1`
normalized to `G PON 1` - the workbook records that vessel as "TK. G. PON 1", so the mangling came
from KLIP - and with only `BG.`/`MT.` counting as cargo, no segment of
`TK. SHERIN 03/TB. PACIFIC STAR I` matched and the fallback took the last one, the tug.

**What this does not change.** No quantity, outstanding or status calculation is touched. The only
visible movement is which vessel a shipment is attributed to, on Shipping Performance, Vessel Idle
and Vessel History.

**Still open.** A new SAP code whose vessel name KLIP does not recognise still creates a master
