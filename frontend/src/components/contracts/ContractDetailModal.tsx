  ...
  const openStoDetail = useCallback(
    (row: StoInfoRow) => {
      ...
      if (row.type === 'shipment') {
        api
          .get('/shipments', {
            params: { sto: row.sto_number, contract: contract.contract_id, limit: 1 },
          })
          .then((res) => {
        ...
        api
          .get('/trucking', { params: { contract: contract.contract_id, limit: 200 } })
          .then((res) => {
            const list =
              ...
              []
            const op =
              Array.isArray(list) && row.operation_id
                ? list.find((o: { operation_id?: string }) => String(o.operation_id || '') === String(row.operation_id))
