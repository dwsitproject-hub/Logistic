...
export const FIELD_HELP = {
  ...
  contractPerfTradeCycle: `Trade Cycle = Due Date Delivery End − Completion Date. Completion Date = ATC; if empty, ETC — and today when the ETC has already passed. On a contract still running, neither → today, so a contract nobody has planned is still measured against its due date. A closed contract with neither stays "-". LAND reads WB as the ATC and Last Planning Delivery as the ETC. Negative = Late.`,
