 ...
 * - Latest row per STO (DISTINCT ON sto_number)
 * - When summed STO movement exceeds ~120% of contract qty, exclude lines that look like
 *   full-contract duplicates (>= 95% of ordered), then fall back to MAX per STO if still inflated
 *   (SAP often repeats cumulative contract receive/delivery on each STO line).
 */
