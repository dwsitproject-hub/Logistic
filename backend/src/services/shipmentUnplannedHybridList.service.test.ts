...
describe('shipmentUnplannedHybridList.service', () => {
  ...
  describe('isAllHybridListRequest', () => {
    it('returns true for ALL and empty status', async () => {
    ...
    it('returns false for pipeline stage filters', async () => {
  ...
  describe('shouldResolveAllHybridShipmentsList', () => {
    it('stays on ALL hybrid for 10-digit PO/STO search so Unplanned backlog remains visible', async () => {
    ...
    it('uses execution list path when Pending ATC filter is active', async () => {
  ...
  describe('shouldResolveCancelledHybridShipmentsList', () => {
    it('keeps Cancelled hybrid for Cancelled card status', async () => {
  ...
  describe('shouldResolveCompletedHybridShipmentsList', () => {
    it('keeps Completed hybrid for 10-digit PO/STO search', async () => {
  ...
  describe('isUnplannedHybridListRequest', () => {
    it('returns true only for UNPLANNED', async () => {
  ...
  describe('buildShipmentAllHybridListContext', () => {
    it('uses toolbar-only execution outer SQL without unplanned predicate', async () => {
    ...
    it('does not share cache between skipSapJoin shell and hydrate so OS/receive/delivery can fill', async () => {
    ...
    it('serializes hydrate hybrid but not the compact skipSapJoin shell', async () => {
      expect(shouldSerializeHybridListQuery(true)).toBe(false);
      expect(shouldSerializeHybridListQuery(false)).toBe(true);
    });

    it('does not share cache across sort keys', async () => {
    ...
    it('hydrate execution SQL still computes OS, receive, and delivery qty', async () => {
      const ctx = buildShipmentAllHybridListContext({ ...baseInput, skipSapJoin: false });
      const q = await buildShipmentListEnrichedPageQuery(ctx.shipmentCtx, 20, 0);
    ...
    it('shell execution SQL omits qty_move so first paint cannot poison hydrate cache', async () => {
      const ctx = buildShipmentAllHybridListContext(baseInput);
      const q = await buildShipmentListEnrichedPageQuery(ctx.shipmentCtx, 20, 0);
    ...
    it('includes unplanned and preplanned contract backlog for ALL hybrid', async () => {
      const ctx = buildShipmentAllHybridListContext(baseInput);
  ...
  describe('buildShipmentUnplannedHybridListContext', () => {
    it('is PO backlog only (no unplanned execution predicate)', async () => {
    ...
    it('defaults server sort to created_at DESC on shipment context', async () => {
      const ctx = buildShipmentUnplannedHybridListContext(baseInput);
      expect(ctx.shipmentCtx.sortKey).toBe('created_at');
      expect(ctx.shipmentCtx.sortDir).toBe('DESC');
    });

    it('passes custom sort to shipment context', async () => {
  ...
  describe('getShipments ALL/Completed/Cancelled hybrid gates', () => {
    it('does not skip hybrid when 10-digit PO/STO search sets exactStoKey', async () => {
